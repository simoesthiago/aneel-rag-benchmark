...
- `5.1-OP` casa `...s-bmodulo-5-1-op...` mas NÃO `...subm5-1...`
...
# "Submódulo 2.4", "Submódulo 2.1A", "Submódulo 5.1-OP", com ou sem acento.
_SUBMODULO_RE = re.compile(
    r"subm[óo]dulo\s+(\d+)\.(\d+)([a-z]?)(?:-([a-z]+))?",
    re.IGNORECASE,
)
...
def query_submodulo_id(query: str) -> str | None:
    ...
    `2-10-rq`), ou `None` se a pergunta não cita um submódulo.
    """
    ...
    sub = match.group(2)
    sufixo = (match.group(3) or "").lower()
    qualificador = (match.group(4) or "").lower()
    identificador = f"{modulo}-{sub}{sufixo}"
    if qualificador:
        identificador = f"{identificador}-{qualificador}"
...
def doc_matches_submodulo(document_id: str, identificador: str) -> bool:
    ...
    PRORET, `rede-`/`s-bmodulo-` em Procedimentos de Rede) e ser seguido por um
    caractere que não seja letra/dígito — assim `2-1` não casa `2-10` nem
    `2-1a`, e strings de versão (`-v-2-5-`) não são confundidas com submódulos.
    """
    if not identificador:
        return False
    identificador = identificador.lower()
    prefixos = (
        r"(?:rede-|s-bmodulo-)" if identificador.count("-") >= 2 else r"(?:subm|rede-)"
    )
    padrao = rf"{prefixos}{re.escape(identificador)}(?![0-9a-z])"
